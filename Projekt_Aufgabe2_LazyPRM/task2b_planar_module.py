...
def animate_workspace_and_cspace(
    ...
    # --- C-Space Obstacles als Background in ax_q ---
    if show_cspace_obstacles:
        (q1min, q1max) = q_limits[0]
        (q2min, q2max) = q_limits[1]

        q1 = np.linspace(q1min, q1max, cspace_occ_n)
        q2 = np.linspace(q2min, q2max, cspace_occ_n)

        occ = np.zeros((cspace_occ_n, cspace_occ_n), dtype=np.uint8)

        # robust: je nach CC-Interface
        def _q_in_collision(q):
        ...
        for i, a in enumerate(q1):
            for j, b in enumerate(q2):
                occ[j, i] = 1 if _q_in_collision([float(a), float(b)]) else 0
